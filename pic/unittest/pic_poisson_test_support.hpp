...
namespace pic_poisson_test
{

inline float64 analytic_solution(float64 kz, float64 ky, float64 kx, float64 kappa2_sum, float64 z,
                                 float64 y, float64 x)
{
  return std::sin(kx * x) * std::sin(ky * y) * std::sin(kz * z) / kappa2_sum;
}

inline float64 analytic_source(float64 kz, float64 ky, float64 kx, float64 z, float64 y, float64 x)
{
...
inline nix::json make_default_config()
{
  nix::json config;
  config["option"]                  = nix::json::object();
...
inline std::array<int, 3> make_rank_coords(int rank, const std::array<int, 3>& proc_dims)
{
...
inline nix::Dims3D make_chunk_grid_dims(const nix::Dims3D& global_dims,
                                        const nix::Dims3D& chunk_dims)
{
  return {global_dims[0] / chunk_dims[0], global_dims[1] / chunk_dims[1],
          global_dims[2] / chunk_dims[2]};
}

inline int flatten_chunk_index(int cz, int cy, int cx, const nix::Dims3D& chunk_grid_dims)
{
  return elliptic::ChunkAccessor::flatten_index(cz, cy, cx, chunk_grid_dims);
}

inline std::array<int, 3> unflatten_chunk_index(int id, const nix::Dims3D& chunk_grid_dims)
{
...
inline int compute_rank_from_chunk_coords(const std::array<int, 3>& chunk_coords,
...
inline std::unique_ptr<ChunkT> make_chunk_impl(const nix::Dims3D& dims, const nix::Bool3D& has_dim,
                                               const nix::Dims3D& global_dims,
                                               const nix::Dims3D& offset, int id, nix::json& config)
{
...
inline std::unique_ptr<TestPicChunk>
make_test_chunk(const nix::Dims3D& dims, const nix::Bool3D& has_dim, const nix::Dims3D& global_dims,
                const nix::Dims3D& offset, int id, nix::json& config)
{
...
inline void set_chunk_neighbors_and_comm(std::vector<std::unique_ptr<PicChunk>>& chunkvec,
                                         const nix::Dims3D&                      chunk_grid_dims,
                                         const std::array<int, 3>&               proc_dims)
{
...
inline void initialize_chunkvec(std::vector<std::unique_ptr<PicChunk>>& chunkvec, int rank,
                                const std::array<int, 3>& proc_dims, const nix::Dims3D& global_dims,
                                const nix::Dims3D& chunk_dims, const nix::Bool3D& has_dim, int mz,
                                int my, int mx, nix::json& config)
{
...
inline void require_src_equals_sol(const std::vector<std::unique_ptr<PicChunk>>& chunks,
                                   float64                                       tol)
{
...
inline void require_rms_error_below(const std::vector<std::unique_ptr<PicChunk>>& chunks, int mz,
                                    int my, int mx, const nix::Dims3D& global_dims, float64 tol)
{
